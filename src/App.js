...
class App extends Component {
  state = {searchInput: '', listDestination: initialDestinationsList}
  ...
  render() {
    const {searchInput, listDestination} = this.state
    const searchResults = listDestination.filter(eachUser =>
      eachUser.name.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return (
      <div className="bg-container">
        <h1 className="heading">Destination Search</h1>
        <input className="input" type="search" onChange={this.inputGiven} />
